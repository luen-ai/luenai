(function(){
  const fmt = new Intl.NumberFormat('ko-KR');
  window.neo = {
    sb(){
      if (!window.supabase) throw new Error('supabase-js 로드 실패');
      const url = window.NEO_SUPABASE_URL;
      const key = window.NEO_SUPABASE_ANON_KEY;
      window.__neo_sb = window.__neo_sb || window.supabase.createClient(url, key);
      return window.__neo_sb;
    },
    won(n){
      const v = Number(n||0);
      return fmt.format(v);
    },
    // 매우 단순한 로컬 로그인(테이블 기반). 운영용 보안 강화는 다음 단계에서.
    setSession(kind, user){
      localStorage.setItem(`neo_${kind}_logged_in`, 'true');
      localStorage.setItem(`neo_${kind}_user`, JSON.stringify(user||{}));
    },
    clearSession(kind){
      localStorage.removeItem(`neo_${kind}_logged_in`);
      localStorage.removeItem(`neo_${kind}_user`);
    },
    getSession(kind){
      const ok = localStorage.getItem(`neo_${kind}_logged_in`) === 'true';
      const raw = localStorage.getItem(`neo_${kind}_user`);
      return { ok, user: raw ? JSON.parse(raw) : null };
    },
    require(kind, redirect){
      const s = window.neo.getSession(kind);
      if (!s.ok) {
        alert('로그인이 필요합니다.');
        window.location.href = redirect;
        return null;
      }
      return s.user;
    }
  };
})();
